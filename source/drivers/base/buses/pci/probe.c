			...
			struct resource *res, unsigned int pos)
{
	...
	res->name = do_get_device_name(dev);
...
//	dev_set_name(&child->dev, "%04x:%02x", pci_domain_nr(child), busnr);
	...
	child->self = bridge;
	child->bridge = pci_dev_get(bridge);
...
int __devinit pci_scan_bridge(struct pci_bus *bus, struct pci_dev *dev, int max, int pass)
{
	...
	dev_dbg(dev, "scanning [bus %02x-%02x] behind bridge, pass %d\n",
								...
								int crs_timeout)
{
	...
	while (*l == 0xffff0001) {
		...
		if (delay > crs_timeout) {
			dev_warn(bus, "pci %04x:%02x:%02x.%d: not "
				"responding\n", pci_domain_nr(bus),
	...
	struct pci_ops *ops, void *sysdata, struct list_head *resources)
{
	...
	*/
	...
	/* host bridge 已经 b的parent，因此引用计数器是对的 */
	b->bridge = bridge;
...
//	pci_create_legacy_files(b);
	...
	printk("PCI host bridge to bus %s\n", do_get_device_name(b));
	...
	list_for_each_entry_safe(window, n, resources, list) {
		...
		if (offset) {
			...
			bus_addr[0] = '\0';
		dev_info(b, "root bus resource %pR%s\n", res, bus_addr);
	}
...
bus_name_err:
	do_unregister_device(b);
	b = NULL;
bus_register_err:
bridge_name_err:
	do_unregister_device(bridge);
	bridge = NULL;
bridge_register_err:
	kfree(bridge);
err_out:
