...
static void __cpuinit probe_pcache(void)
{
	...
	default:
		if (!(config & MIPS_CONF_M))
			hal_panic("Don't know how to probe P-caches on this cpu.");
	...
	if ((prid & 0xff00) == PRID_IMP_R4000 && (prid & 0xff) < 0x40 &&
	    ...
	    PAGE_SIZE <= 0x8000)
		hal_panic("Improper R4000SC processor configuration detected.");
...
extern int loongson2_sc_init(void);
static void __cpuinit setup_scache(void)
{
	...
	case CPU_LOONGSON2:
		scache_size = loongson2_sc_init();
		hal_printf("Unified secondary cache %ldkB %s, linesize %d bytes.\n",
...
#endif
		hal_printf("Lx Unified secondary cache %ldkB %s, linesize %d bytes, waybit=%d.\n",
	...
	default:
		if (c->isa_level == MIPS_CPU_ISA_M32R1 ||
			...
			if (!(c->scache.flags & MIPS_CACHE_NOT_PRESENT))
				hal_panic("Dunno how to handle MIPS32 / MIPS64 second level cache");
#endif
			return;
		}
		sc_present = 0;
...
static void __cpuinit coherency_setup(void)
{
	...
	hal_printf("Using cache attribute %d\n", cca);
