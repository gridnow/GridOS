		       ...
		       const fde *x, const fde *y)
{
	...
	temp.p = (unsigned char *)x->pc_begin;
	_Unwind_Ptr x_ptr = *(temp._un_p);
  	temp.p = (unsigned char *)y->pc_begin;
	_Unwind_Ptr y_ptr = *(temp._un_p);
...
add_fdes (struct object *ob, struct fde_accumulator *accu, const fde *this_fde)
{
  	...
  	for (; ! last_fde (ob, this_fde); this_fde = next_fde (this_fde))
    {
		...
		if (encoding == DW_EH_PE_absptr)
		{
			...
			temp.p = (unsigned char *)this_fde->pc_begin;
	  		if (*(temp._un_p) == 0)
	    		continue;
		}
      	else
...
linear_search_fdes (struct object *ob, const fde *this_fde, void *pc)
{
  	...
  	for (; ! last_fde (ob, this_fde); this_fde = next_fde (this_fde))
    {
      	...
      	if (encoding == DW_EH_PE_absptr)
		{
			...
			temp.p = (unsigned char *)this_fde->pc_begin;
	  		pc_begin = (temp._un_p)[0];
	  		pc_range = (temp._un_p)[1];
	  		if (pc_begin == 0)
	    		continue;
		}
      	else
		{
	  		...
	  		p = read_encoded_value_with_base ((unsigned char)encoding, base,
					    this_fde->pc_begin, &pc_begin);
	  		read_encoded_value_with_base ((unsigned char)(encoding & 0x0F), 0, p, &pc_range);
...
binary_search_unencoded_fdes (struct object *ob, void *pc)
{
  	...
  	for (lo = 0, hi = vec->count; lo < hi; )
    {
		...
		temp.p = (unsigned char *)f->pc_begin;
      	pc_begin = (temp._void_p)[0];
      	pc_range = (temp._uaddr_p)[1];
  	...
  	for (lo = 0, hi = vec->count; lo < hi; )
    {
		size_t i = (lo + hi) / 2;
      	...
      	p = read_encoded_value_with_base (encoding, base, f->pc_begin,
					&pc_begin);
      	read_encoded_value_with_base (encoding & 0x0F, NULL, p, &pc_range);
