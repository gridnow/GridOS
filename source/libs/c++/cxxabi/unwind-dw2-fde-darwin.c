...
static struct km_object_info *g_object_info = NULL;
static struct live_images images_live;
static struct live_images *g_images = &images_live;
/**
	live-image , object_info get
	TODO,在g_image需要在动态链接load的时候动态的添加到g_images内
*/
void *_keymgr_get_and_lock_processwide_ptr (int key)
{
	switch (key)
	{
		case KEYMGR_GCC3_DW2_OBJ_LIST:
			return g_object_info;
		case KEYMGR_GCC3_LIVE_IMAGE_LIST:
			return g_images;
		default:
			return NULL;
	}
...
void _keymgr_set_and_unlock_processwide_ptr (int key, void *obj)
{
	switch (key)
	{
		case KEYMGR_GCC3_DW2_OBJ_LIST:
...
void _keymgr_unlock_processwide_ptr (int key)
{
	//TODO
	return;
}

static void *dlobj = NULL;
/**
	用于获取目标文件的sections段起始地址
*/
char *getsectdatafromheader (struct mach_header *header, const char *sections, \
			const char *name, unsigned long *size)
{
	if (!dlobj)
	{
		const char *file = "0:/os/i386/test_new";
		dlobj = dlopen(file, 0);
		if (!dlobj)
		{
			return NULL;
		}
		return dlsection_vaddr(dlobj, name, size);
	}
	return NULL;
}

...
/* We may use a array to recoder it,than return it*/
  ...
  for (; image != NULL; image = image->next)
	...
	fde = getsectdatafromheader_64 ((struct mach_header_64 *) image->mh,
				     "__DATA", ".eh_frame", &sz);
#else
	fde = getsectdatafromheader (image->mh, "__DATA", ".eh_frame", &sz);
#endif
	if (fde == NULL)
	  {
#ifdef __ppc64__
...
_Unwind_Find_FDE (void *pc, struct dwarf_eh_bases *bases)
{
  struct km_object_info *the_obj_info;
  const fde *ret = NULL;
/* also recorder it by arry,index is a cpuid */
  the_obj_info =
    _keymgr_get_and_lock_processwide_ptr (KEYMGR_GCC3_DW2_OBJ_LIST);
  if (! the_obj_info)
